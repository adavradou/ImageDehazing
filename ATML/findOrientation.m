...
while(~found)
    ...
    while(nGood < 10 && sum(good)>0)
        ...
        if(nGood>=10)
            ...
            [normals,points,locations,deleted] = deleteParallelVecs(normals,points,locations,goodInds,ang);
            ...
            for i=1:size(normals,1)
                good = good & dot(N,repmat(normals(i,:),size(N,1),1),2)<=cos(deg2rad(ang));
            end
...
for i=1:nGood
    for j=i+1:nGood
        ...
        sig = sign(curA);
        if(norm(curA) > 10^-7 && ~any(sig-sig(1))) %if planes are not parallel, and A is all positive direction (or all negative)
            counter = counter + 1;
...
for i=sz:-1:1
    for j=1:i-1
        if(dot(N(i,:),N(j,:))>cos(deg2rad(ang)))%if the angle is less than 15
            N(i,:) = [];
            P(i,:) = [];
            L(i,:) = [];
