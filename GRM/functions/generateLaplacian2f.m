function [ T,L ] = generateLaplacian2f( I,T_est,sig,L)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     0.3015
C = cell2mat(C);
x_ind=1:D*windows_num;
x_ind=reshape(x_ind,D,windows_num);
x_ind=repmat(x_ind,D,1);  % 3   for rgb
x_ind=x_ind(:)';
y_ind=repmat(1:3*windows_num,D,1);
y_ind=y_ind(:)';
cov_inv_rgb = sparse(x_ind,y_ind,C(:)',D*windows_num,D*windows_num);
clear C
