...
%     [x, y] = ginput(1);
    ...
    min = round(size(HazeImg,2)/4); %9/10/2018: Agapi changed to not manually choose the hazy pixels. Random selection.
    max = min*3; %We assume that there is no haze close to the corners.
    x = randi([min max], 1, 1);
    min = round(size(HazeImg,1)/4);
    max = min*3;
    y = randi([min max], 1, 1);
        
    A = HazeImg(round(y), round(x), :);
%     A = min(A, 255); %12/10/2018: Agapi commended it due to "Index
%     exceeds matrix dimensions" error.
%     close(h);
