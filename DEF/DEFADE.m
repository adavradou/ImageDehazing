    ...
    % Artusi 25/10/208: converted to a function  
   
    function [dh_img] = DEFADE(h_img)
    %% Basic setup         
        [row col dim]                   = size(h_img);         
        Ig                              = double(rgb2gray(h_img));                    
        R                               = double(h_img(:,:,1));
        G                               = double(h_img(:,:,2));
        B                               = double(h_img(:,:,3));
        L                               = (R+G+B)/3; 
    ...
    %% Execute FADE using 8 x 8 overlapped patches           
        [D_o D_map_o]                   = FADE_overlapped(h_img);
...
%% Preprocessed images
      ...
      % I3: Fog aware contrast enhancement  
            ...
            for ii=1:50
                ...
                fi_diff(ii)              = mean2(L) - fi_intensity;
            ...
            fi_intensity_RGB(3)         = mean(Bfi(fi));                                       
            fi_intensity                = mean([Rfi(fi); Gfi(fi); Bfi(fi)]);                 
        I3                              = uint8(2.5*(double(img_hazy)-fi_intensity));
...
%% Multiscale refinement              
      ...
      % Multiscale refinement: multiresolution blending
        for iii = 1:N            
            ...
            pyrW                        = gaussian_pyramid(W(:,:,iii),pyr_N);
            % Laplacian pyramid decomposition for Pre_I  
            pyrI                        = laplacian_pyramid(Pre_I(:,:,:,iii),pyr_N);
            % Fused pyramid
            for l = 1:nlev
...
%         baseFileName = 'DEFADE.jpg'; 
%         fullFileName = fullfile(folder_results, baseFileName);
%         imwrite(dehazed_image, fullFileName);

        clear Defogged_I, clear W_fog, clear Pre_I
%         figure; imshow(Defogged_I);title('Defade');
